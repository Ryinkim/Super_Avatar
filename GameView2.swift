...
struct GameView2: View {
    ...
    let captureFrame = CGRect(x: 670, y: 600, width: 2000, height: 1500)
   
    // gameView & state button
    @State private var isGameView2Visible = true
    ...
    @State private var isButtonEnding = false
        ...
        VStack {
            ...
            if let screenshot = screenshot {
                              Image(uiImage: screenshot)
                                  ...
                                  .position(x: 705, y: 260)
                                  .padding()
                                  .zIndex(5)
                Image("polaroid")
                    .resizable()
                    .frame(width: 1280, height: 720)
                    .position(x: 508, y: 153)
                    .zIndex(6)
    ...
    private func captureScreenshot() {
            let captureFrame = CGRect(x: -40, y: 190, width: 800, height: 600)
            ...
            let renderer = UIGraphicsImageRenderer(size: captureFrame.size)
            screenshot = renderer.image { _ in
                window.drawHierarchy(in: captureFrame, afterScreenUpdates: true)
            }
